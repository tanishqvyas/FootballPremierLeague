...
#Initializing player chemistry
player_names=(Player_RDD.select('name')).collect()
columns=['player1','player2','chemistry']

player_chemistry = spark.createDataFrame([player_names[0][0], player_names[1][0], 0.5], columns)
for j in range(i+2,len(player_names)):
	newRow = spark.createDataFrame([player_names[0][0],player_names[i][0], 0.5],columns)
	player_chemistry = player_chemistry.union(newRow)
	
for i in range(1,len(player_names)):
	for j in range(i+1,len(player_names)):
		newRow = spark.createDataFrame([player_names[i][0],player_names[j][0], 0.5],columns)
	player_chemistry = player_chemistry.union(newRow)
...
def same_team_chem(player1,player2):
	...
	#player_chemistry=player_chemistry.withColumn("chemistry",F.when(F.col("Id")==player,to_insert).otherwise(F.col("passAcc")))
	#return chemistry
...
def calc_metrics(rdd):
	...
	for data in rdds:
		...
		if 'eventId' in data:
				...
				for i in stored['teamsData']:
					...
					if stored['teamsData'][i]['hasformation']==1:
						...
						for j in playedtime:
							...
							contrib=Player_RDD.select("contribution",F.when(F.col("Id")==j)
							foul=Player_RDD.select("numFouls",F.when(F.col("Id")==j)
							own_goal=Player_RDD.select("ownGoals",F.when(F.col("Id")==j)							
							playerPerformance=contrib-(0.005*foul)-(0.05*own_goal)
			...
			team1=stored['teamsData'][0]
			team2=stored['teamsData'][1]

			lineup_1=[i['playerId'] for i in team1['formation']['lineup']]
			substitutions_1=[i['playerId'] for i in team1['formation']['substitutions']]

			lineup_2=[i['playerId'] for i in team2['formation']['lineup']]
			substitutions_2=[i['playerId'] for i in team2['formation']['substitutions']]
